...
function dynamic(x::State, Pbt::Float64,Pnl_next::Float64, mg)
    ...
    x_next = State(Pnl_next, LoH_next, E_bt_next, δ_next)
    Pshed = Pnl - Pbt - PH2
...
function cost_to_go(pen::Float64,Pshed::Float64,Ph2::Float64,Pbt::Float64,δ::Int64,fc::ProductionUnit,el::ProductionUnit,bt::Battery,d_rt::Float64,d_st::Float64)
    ...
    c+= pen * max(0.0, Pshed)
    if Ph2> ( 10e-12)
            c += (d_rt/0.1)*(fc.power_rated*fc.investment_price)
        if δ !=1
            c += (d_st/0.1)*(fc.power_rated*fc.investment_price)
        end
    elseif (Ph2<  -10e-12)
         c += (d_rt/0.1)*(el.power_rated*el.investment_price)
        if δ !=-1
            c += (d_st/0.1)*(el.power_rated*el.investment_price)
        end
    end
...
TBW
"""
function DPrecursion(K::Int64, pen::Float64,  max_pnl::Float64, min_pnl::Float64, ref::Vector{Float64},σ::Float64,ϕ::Float64, mg,d_rt::Float64=deg_ratio_rt,d_st::Float64=deg_ratio_st)
            ...
            @inbounds for loh = 1:nloh, ebt = 1:nEbt,δ =1:3
                ...
                @inbounds for u_i = 1:nu  #gérer les situations où il n'y pas de u
                    ...
                    @inbounds @fastmath for iw in eachindex(demand) # for each uncertainty value                
                     ...
                     Jk_xu += cost_to_go(pen,Pshed,PH2,uk,xk.δ,fc,el,bt,d_rt,d_st)  #+ max(x_next.δ,0)*(d_rt/1-Eol)*(fc.investment_price)
...
function DPrecursion_jopt(K::Int64, pen::Float64,ref::Vector{Float64},σ::Float64,ϕ::Float64, Pnl_cate,mg,d_rt::Float64=deg_ratio_rt,d_st::Float64=deg_ratio_st)
            ...
            @inbounds for loh = 1:nloh, ebt = 1:nEbt,δ =1:3
                ...
                @inbounds for u_i = 1:nu  #gérer les situations où il n'y pas de u
                    ...
                    @inbounds @fastmath for iw in eachindex(demand) # for each uncertainty value                
                     ...
                     Jk_xu += cost_to_go(pen,Pshed,PH2,uk,xk.δ,fc,el,bt,d_rt,d_st)
            ...
            @inbounds for ebt = 1:nEbt,δ =1:3
                ...
                @inbounds for u_i = 1:nu  #gérer les situations où il n'y pas de u
                    ...
                    x_next, Pshed, PH2 = dynamic(xk, uk, 0.0, mg)
                        ...
                        Jk_xu = Jopt_next( x_next.LoH, x_next.Ebatt, x_next.δ) + cost_to_go(pen,Pshed,PH2,uk,xk.δ,fc,el,bt,d_rt,d_st)
    ...
    @inbounds for k = K:-1:1
        ...
        @inbounds  @threads for pnl = 1:nPnl
            ...
            @inbounds for loh = 1:nloh, ebt = 1:nEbt,δ=1:3
              ...
              #  @assert Jk_xu_min >= Jopt[pnl, loh, ebt, δ, K+1] - 0.1 " problem, $m, $(h+1), $cat,$xk, $(dynamic(xk, u_opt, 0.0, mg)), $u_opt,$( Jk_xu_min), $(Jopt[pnl,loh,ebt,cy,rt1,rt2,K+1])"
...
function DPrecursion_uopt(K::Int64, pen::Float64,  max_pnl::Float64, min_pnl::Float64, ref,σ,ϕ, mg,d_rt::Float64=deg_ratio_rt,d_st::Float64=deg_ratio_st)
  el=mg.electrolyzer[1]
    fc= mg.dispatchables.fuel_cell[1]
    bt = mg.storage
    hytank = mg.tanks.h2Tank
...
function DPrecursion_rb_cost(K::Int64, pen::Float64, max_pnl::Float64, min_pnl::Float64,ref,σ,ϕ, mg,d_rt::Float64=deg_ratio_rt,d_st::Float64=deg_ratio_st)
    el=mg.electrolyzer[1]
    fc= mg.dispatchables.fuel_cell[1]
    bt = mg.storage
    hytank = mg.tanks.h2Tank
...
function sdp_simu_jopt(Jopt, n, pen,ϕ,σ,Pnl_cate, mg1,ref::Vector{Float64},Pnl::Vector{Float64},d_rt::Float64=deg_ratio_rt,d_st::Float64=deg_ratio_st,data=data)
   ...
   el=mg1.electrolyzer[1]
    ...
    for k = 1:n
        ...
        err=(xk.Pnl-ref[k])
        nu = u_range!(Pbt_grid, xk, mg1, 5, 49, 49)
        ...
        @inbounds for u_i = 1:nu  #gérer les situations où il n'y pas de u
           ...
           x_next, Ps, Ph2 = dynamic(xk, uk, 0.0, mg1)
             ...
             Jk_xu += cost_to_go(pen,Ps,Ph2,uk,xk.δ,fc,el,bt,d_rt,d_st)
            if (Jk_xu < Jk_xu_min)
                u_opt = uk
                Jk_xu_min = Jk_xu
                ph_opt = Ph2
...
function Dp_simu(Jopt, n, pen, mg1,Pnl::Vector{Float64},d_rt::Float64=deg_ratio_rt,d_st::Float64=deg_ratio_st,data=data)
    ...
    for k = 1:n
        ...
        @inbounds for u_i = 1:nu  #gérer les situations où il n'y pas de u
           ...
           x_next, Ps, Ph2 = dynamic(xk, uk, 0.0, mg1)
                ...
                Jk_xu = J_interp( x_next.LoH, x_next.Ebatt, x_next.δ) + cost_to_go(pen,Ps,Ph2,uk,xk.δ,fc,el,bt,d_rt,d_st)
