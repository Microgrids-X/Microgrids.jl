...
function cost_to_go(pen::Float32,Pshed,Ph2,Pbt,δ,fc,el,bt,d_rt::Float32=deg_ratio_rt,d_st::Float32=deg_ratio_st,Eol::Float32=0.10f0)
    ...
    if Pbt<=-1e-3 || Pbt>=1e-3
        c+=(abs(Pbt)*bt.investment_price)/(Int32(2)*bt.lifetime_cycles)
    end
    return c
end
function cost_to_go2(pen::Float32,Pshed::Float32,Ph2::Float32,Pbt::Float32,δ::Int32,fc,el,bt)
    c=0.f0
    d_rt=Float32(1.5e-6/0.7)
    d_st=Float32(14e-6/0.7)
    Eol=0.10f0
...
function DPrecursion_jopt_mc(K, pen, time_init, mi_l, Pnl_cate, Pnl_prob, mg;mode=1)
...
function sdp_simu_jopt_mc(Jopt, n, pen, mg1, Pnl_cate, Pnl_prob,Pnl;mode::Int32=Int32(1),data=data)
   ...
   Pren = production(mg1.nondispatchables[1]) .+ production(mg1.nondispatchables[2])
    ...
    Pel = zeros(Float32,n)
    Pfc = zeros(Float32,n)
    Ebt = zeros(Float32, n+ 1)
   
    Loh = zeros(Float32,n + 1)
    Lof = zeros(Float32,n + 1)
    Pgen = zeros(Float32,n)
    Phb = zeros(Float32,n)
    Pshed = zeros(Float32,n)
    Pdump = zeros(Float32,n)
    Pspill = zeros(Float32,n)
    ...
    for k = 1:n
      ...
      #  @assert cat >= 1 && cat <= 16 "error $cat, $(xk.Pnl),  $(h+1), $m, $(Pnl_cate[m,h+1,1]), $(xk.Pnl-Pnl_cate[m,h+1,1]), $(Pnl_cate[m,h+1,2]-Pnl_cate[m,h+1,1]))"
        ...
        @inbounds for u_i = 1:nu  #gérer les situations où il n'y pas de u
            uk = Pbt_grid[u_i]
            Jk_xu = 0.0f0
            LoH_next, E_bt_next, δ_next, Ps, Ph2 = dynamic(xk, uk, 0.0f0, fc,el,hytank,bt,dt)

            @fastmath for iw = 1:n_cat # for each uncertainty value  
                if Pnl_prob[m, h+1, iw, cat] != 0.0f0
                    if mode==1
                        Jk_xu += J_interp(Pnl_cate[m_next, h_next+1, iw], LoH_next,  E_bt_next, δ_next) * Pnl_prob[m, h+1, iw, cat]
                    elseif mode==2
                        Jk_xu += J_interp((Pnl_cate[m_next, h_next+1, iw]+Pnl_cate[m_next, h_next+1, iw+1])/2, LoH_next,  E_bt_next, δ_next) * Pnl_prob[m, h+1, iw, cat]
                    elseif mode==3 
                         Jk_xu += J_interp(Pnl_cate[m_next, h_next+1, iw+1], LoH_next,  E_bt_next, δ_next) * Pnl_prob[m, h+1, iw, cat]
                    elseif mode==4
                        Jk_xu += (J_interp(Pnl_cate[m_next, h_next+1, iw], LoH_next,  E_bt_next, δ_next)+J_interp(Pnl_cate[m_next, h_next+1, iw+1],LoH_next,  E_bt_next,δ_next)) * Pnl_prob[m, h+1, iw, cat]/2
                    end
                end
            end # each uncertainty
       ...
       LoH_next, E_bt_next, δ_next, Ps, Ph2 = dynamic(xk, u_opt, 0.0f0, fc,el,hytank,bt,dt)
        Pbt[k] = u_opt
        Pel[k] = ifelse(Ph2 > 0.0f0, 0.0f0, -Ph2)
        Pfc[k] = ifelse(Ph2 > 0.0f0, Ph2, 0.0f0)
        Ebt[k+1] = E_bt_next
